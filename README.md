...
| **32cls** | 各状態を約10フレームで MCU 追加学習 | **99.1%** | **100%** |
| 32cls（参考） | 追加学習前（出荷直後） | 77.8% | 81.2% |

いずれも **m=32 / D=167 / AI RAM ~11KB**＝実機（SRAM 16KB / Flash 256KB）に収まる。移植元 IchiPing CNN も現地校正で 32cls 最大 100% を達成しており、本移植版は**同等クラスの精度を、学習パラメータ約 1/84・16KB MCU・オンデバイス学習で実現**する点に価値がある。
...
sim_export/  成果物（ベスト版のみ）
  ...
  model1_BEST_32cls_ondevice/   32cls MCU追加学習モデル（β=32×32）
...
実機ではスピーカ・マイクが変更されるため **データは再取得**。本リポジトリのパイプライン（クロスベースライン×3・オーグメンテーション×3）をそのまま流用して β を再学習し、14cls は焼くだけ／32cls は MCU 追加学習で運用する。詳細はレポート §8 参照。
