...
1. IR warp (推奨, 励振 PRBS が既知のデータ用)
   ...
   baseline 側はワープしないので「気温が変わった後に古い baseline で推論する」失敗モードが
   そのまま再現される (UNO Q 方式と同じ考え方)。
2. feature warp (励振が再現不能なデータ用: IchiPing FRDM 世代は xorshift seed 非再現)
...
class IRWarper:
    ...
    def warp_ir(self, h: np.ndarray, eps: float) -> np.ndarray:
        t = np.arange(len(h), dtype=np.float64) - self.pre
        return np.interp(t * (1.0 + eps), t, h, left=0.0, right=0.0)

    def warp_audio(self, a: np.ndarray, eps: float) -> np.ndarray:
        """周波数を (1+ε) 倍にスケールした録音を返す (ε=0 で恒等)。"""
        if eps == 0.0:
            return a.copy()
        h = self.impulse_response(a)
        return a + self.reconvolve(self.warp_ir(h, eps) - h)
...
def warp_db_spectrum(db: np.ndarray, eps: float) -> np.ndarray:
...
def fit_warp(ref_db: np.ndarray, target_db: np.ndarray, lo: int, hi: int,
             grid=np.linspace(-0.06, 0.06, 241)) -> float:
    """target ≈ warp(ref, ε) となる ε を格子探索 (bin 範囲 [lo,hi) の相関最大)。"""
    best, best_c = 0.0, -np.inf
    for e in grid:
        w = warp_db_spectrum(ref_db, float(e))[lo:hi]
